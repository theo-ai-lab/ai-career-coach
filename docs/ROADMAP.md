...
| X2 | **Fix `userId = resumeId` aliasing → real per-user identity** | A returning user resumes their *own* history; no cross-session leak (red-team #3 — the "anxiety you mentioned" fabrication) | M–L | High | Documented at `route.ts:99-105`; mitigated for evals via `skipMemory` only |
